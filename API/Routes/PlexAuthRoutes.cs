...
public static class PlexAuthRoutes
{
    ...
    {
        app.MapGet("/auth/plex/url", async (PlexOAuthService plexService) =>
        {
            var controller = new PlexAuthController();
            return await controller.GetPlexOAuthUrl(plexService);
        });

        app.MapGet("/auth/plex/callback", async (HttpContext context, PlexOAuthService plexService, AppDbContext db, JwtService jwtService) =>
        {
            var controller = new PlexAuthController();
